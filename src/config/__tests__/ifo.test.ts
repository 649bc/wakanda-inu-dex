...
describe('Config IFOs', () => {
  it.each(ifos.map((ifo) => ifo.id))('ISPO %s has an unique id', (id) => {
    const duplicates = ifos.filter((i) => id === i.id)
    expect(duplicates).toHaveLength(1)
  })
  it.each(ifos.map((ifo) => [ifo.id, ifo.address]))('ISPO %s has an unique address', (id, address) => {
    const duplicates = ifos.filter((i) => address === i.address)
    expect(duplicates).toHaveLength(1)
  })
  it.each(ifos.map((ifo) => [ifo.id, ifo.campaignId]))('ISPO %s has an unique campaign ID', (id, campaignId) => {
    const duplicates = ifos.filter((i) => campaignId === i.campaignId)
    expect(duplicates).toHaveLength(1)
  })
  it.each(ifos.map((ifo) => [ifo.id, ifo.campaignId]))('ISPO %s has an existing campaign ID', (id, campaignId) => {
